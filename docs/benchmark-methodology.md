...
构建同一个 image/configuration，保持 Worker seeds 固定，先 warm up 系统，然后每个 case 至少运行三次。保留 loadgen JSON、`/metrics`、命令行、Go version、机器信息、scheduler 和 Worker configuration。不要混合不同硬件上的结果。
...
Stage 3 和 4 的测量只使用单节点 5xA100 80GB 目标环境。Static scaling 使用 1、2、3、4、5 个 Workers，并报告 throughput、per-GPU throughput、scaling efficiency、TTFT、TPOT、P95/P99、cache-hit ratio 和 scheduler overhead。Dynamic 与 interference experiments 在 `stage4-benchmark-plan.md` 中说明。
