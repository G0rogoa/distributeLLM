...
Stage 3 `AffinityIndex` 是单独、更薄的一层。它只记录某个请求最近成功通过了一个真实 worker instance。Entries 有短 TTL，并按 worker instance 和 cache identity 建 key，因此 worker 带着新的 `instance_id` 重启后不会继承旧 affinity。Shadow matches 会在 lifecycle records 中标记为 `ShadowEstimated`，并且只能作为 scheduler bonus。
...
- `remote`：为未来 HTTP tokenizer sidecar 预留。

Go Controller 不重新实现 Hugging Face tokenizer。未来 remote tokenizer 应使用显式 tokenizer name 或 path，默认关闭 `trust_remote_code`，包含 tokenizer revision 和 chat template identity，并在失败时回退到 cache-unaware scheduling。Prompt 内容不能写入日志。
