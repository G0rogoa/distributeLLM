...
Stage 3D 的正式结果必须使用独立 `artifacts/<experiment-id>/` 目录，包含 manifest、requests JSONL、decisions、worker samples、controller/vLLM metrics、`nvidia-smi` 前后快照、summary、analysis 和 logs。ECT 使用冻结 cost profile 做核心对照；在线 EWMA 只能作为附加实验。Workload 分为 unique、shared-prefix、mixed 和 congestion，prefix 长度覆盖 256 到 8192 tokens。报告时分别列出 workload，给出三次重复均值、标准差、样本足够时的 bootstrap 95% CI，以及相对 round-robin/least-loaded 的提升；样本不足时必须明确说明。
